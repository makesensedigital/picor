...
| **DEBT-14** | **The performance and accessibility floors do not block the gate**, and no `lighthouserc.json` exists. The step runs and reports; it cannot fail the build | 0 (gate configuration) | Any real floor failed while `index.html` shipped 31.58 MB, and a step red from day one is a step somebody switches off in a week. **Its trigger fired on 2026-08-12 when DEBT-02 landed** — so the remaining reason is narrower and temporary: the floors must come from a measured score, and until 2026-08-12 the step printed nothing but "Done running autorun". A number nobody can see cannot move | The floors are advisory, so a regression between now and the next commit here would not be caught. This is the one place the gate is weaker than the template's | **Read the scores from the next CI run** — the step now prints the median of three runs plus LCP, CLS and TBT, and keeps the report as an artifact. Put the floors just under those numbers in `lighthouserc.json` and delete `continue-on-error` **in the same commit** | Juan Torresel | 2026-08-11 |
