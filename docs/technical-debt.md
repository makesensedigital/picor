...
| **DEBT-10** | **`GTM_SETUP.md` is published at `https://picor.com.ar/GTM_SETUP.md`** — internal setup documentation, served because the repository is the web root. And there is **no published not-found document**: a wrong URL returns Apache's default error page | 2 | Neither is dangerous. `GTM_SETUP.md` was read during adoption and contains no credential — only the container id, which ships in the markup anyway. A `404.html` is a new page and belongs with the migration that changes how not-found is served | Internal material at a public URL invites the next internal file being committed the same way. The default error page is a dead end with no route back to the site | With OPEN-03. `.gitignore` is the mechanism for the first; a published `404.html` for the second. **Note that deleting `GTM_SETUP.md` does not unpublish it** — it stays in caches, indexes and history | Juan Torresel | 2026-08-11 |
...
**Total: 14 entries, 49 of the 66 baseline findings.**
