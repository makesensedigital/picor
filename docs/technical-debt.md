...
| **DEBT-04** | **Google Fonts is contacted on first render**, from `fonts.googleapis.com` and `fonts.gstatic.com`, before any consent choice exists | 3 | Self-hosting the typefaces is the right fix and needs the font files, a `@font-face` block and a visual check. It is an hour, not a minute | Every visitor's address reaches a third party before they choose anything, and with no server there is nothing to proxy it through. It also blocks first render on a second origin | With OPEN-01 (the consent decision) — the two are the same conversation. **It is now also the top performance item**: Lighthouse measures LCP at 6.8 s and the render-blocking font stylesheet on a third-party origin is the largest remaining cause, so the privacy fix and the speed fix are the same commit. Tighten the performance floor to 0.75 when it lands. **Do not close this by adding the origins to the allowlist**; that converts a finding into a decision nobody took | Juan Torresel | 2026-08-11 |
...
**Open: 10 entries, 22 findings.** Five entries were paid down on 2026-08-11/12 — see below. The
...
| **DEBT-14** | **The performance and accessibility floors did not block the gate**, and no `lighthouserc.json` existed | Floors set from the **median of three measured CI runs**, not guessed — performance 64, accessibility 97, best-practices 100, SEO 100, CLS 0, TBT 10 ms — each with the reason it sits where it does, and `continue-on-error` removed in the same commit. One assertion is a warning rather than an error, `largest-contentful-paint`, and the file says why in place: at 6.8 s it fails any honest floor, and the largest remaining cause is DEBT-04's render-blocking third-party stylesheet | 2026-08-12 |
